...
type_* global;	    // указатель области памяти

type_* GridX;		// сетка для Х
type_* GridY;		// сетка для У

type_* di;		    // диагональ исходной матрицы			
type_* ggl;		    // нижний треугольник исходной матрицы
type_* ggu;		    // верхний треугольник исходной матрицы		

type_* r;			// вспомогательный вектор
type_* x;			// вектор решения
type_* f;			// вектор правой части
type_* p;			// вспомогательный вектор
type_* z;			// вспомогательный вектор
type_* q;			// вспомогательный вектор				

type_* L;			// нижний треугольник обусловленной матрицы
type_* U;			// верхний треугольник обусловленной матрицы

type_* diag;		// диагональ обусловленной матрицы
type_* s;			// вспомогательный вектор				
...
void assembling() {
	...
	type_	lambda, gamma, px, y, xp, yp, hx, hy;	// параметры КЭ
	type_	tmp, hx2, hy2, ud, u1, u2, u3;

	type_	fv[4];		// значения правой части
	type_	B[4][4];	// матрица жёсткости
	type_	C[4][4];	// матрица масс
	type_	F[4];		// вектор правой части

	// Процедура сборки глобальной матрицы
	for (k = 0; k < Ny - 1; k++)
		for (i = 0; i < Nx - 1; i++)	{
...
void AddToMatrix(int i, int j, type_ el)
{
	int k;
	if (i == j) di[i] += el;
	else	{
...
void GaussL(type_* in, type_* out)
{
	int i, j;
	type_ result;
	for (i = 0; i < N; i++)
...
void GaussU(type_* in, type_* out)
{
	int i, j;
...
void MultMatrixOnVector(type_* in, type_* out)
{
	int i, j;
	type_* out1;
	out1 = new type_[N];
...
type_ ScalarMult(type_* v1, type_* v2) {
	int i;
	type_ result;
	result = 0;
	for (i = 0; i < N; i++) {
...
void run()
{
	int iter;
	int i, check, stop;
	type_ alpha, alphazn, alphach, beta, betach, betazn, CheckExit;
